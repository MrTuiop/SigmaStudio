...
- Создайте проект Angular с помощью ng: `ng new sigmastudio.client --defaults --skip-install --skip-git --no-standalone `.
...
- Обновите компонент app.ts для получения и отображения сведений о погоде.
- Измените app.spec.ts с помощью обновленных тестов.
- Обновите app-module.ts, чтобы импортировать HttpClientModule.
- Создание файла проекта (`sigmastudio.client.esproj`).
